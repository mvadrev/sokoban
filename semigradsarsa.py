...
class SemiGradientSarsa():
    ...
    def getBestPolicyAndValue(self):
            for i in range(np.array(self.env.currentState).shape[0]):
                            for j in range(np.array(self.env.currentState).shape[1]):
                                acs = [0,1,2,3]
                                acs_vals = []
                                if(np.array(self.env.currentState)[i][j] == 0):
                                    self.values[i][j] = -20
                                    pass
                                else:
    ...
    def train(self,timesteps):
        ...
        for timestep in range(timesteps):
             ...
             while (self.done == False):
                ...
                position_player_s = np.argwhere(np.array(initialState) == 5)

                position_player_sPrime = np.argwhere(np.array(next_state) == 5)
                ...
                if(self.isTerminalState(next_state)):
                    ...
                    # For all other states we are showing policy evolution as matrix. In the last step we show matplotlib plot
                    plt.figure()
                    ax = sns.heatmap(self.values, linewidths=.5, vmin=-21, vmax=1)
                    plt.show()
